...
export default function UserWithdrawalForm({ onClose }) {
  ...
  const minUserCanWithdraw = isNaira
    ? quizConfig.minUserCanWithdrawNaira
    : quizConfig.minUserCanWithdraw;

  const maxUserCanWithdraw = isNaira
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg w-[95%] max-w-2xl mx-auto my-10 max-h-[90vh] overflow-y-auto">
        ...
        <form className="mt-6 space-y-5" onSubmit={submitHandler}>
          ...
          <div>
            ...
            <p className="text-sm text-red-500 font-bold mb-5 bg-yellow-400 p-3">
              Minimum you can withdraw is {isNaira
                ? `₦${quizConfig.minUserCanWithdrawNaira}`
                : `$${quizConfig.minUserCanWithdraw}`}
            </p>
